$(document).ready(function () {
  ...
  $('.right_float li').on('click',function(){
    $(this).addClass('active').siblings().removeClass('active')
    var index = $(this).index();
    // console.log(index)
    $('.com-list-wrp:eq('+index+')').removeClass('hide').siblings().addClass('hide')
  })

  $('.main-top-part .router').each(function(){
    var height = $(this).height()
    //超出三行
    if (height>66) {
      $(this).height(66).addClass('ellipsis')
    }
  })
});
